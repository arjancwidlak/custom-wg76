...
sub _computePostCount {
	my $self = shift;
	return scalar @{$self->getLineage(['descendants'], {includeOnlyClasses => ['WebGUI::Asset::IssuePost']})};
}

#-------------------------------------------------------------------
sub _computeThreadCount {
	my $self = shift;
	return scalar @{$self->getLineage(['children'], {includeOnlyClasses => ['WebGUI::Asset::IssuePost::IssueThread']})};
}
...
sub appendPostListTemplateVars {
	...
	foreach my $row (@$page) {
		...
		my %lastReply;
		my $hasRead = 0;
		if ($post->get("className") =~ /IssueThread/) {
		...
		if ($row->{className} =~ m/^WebGUI::Asset::IssuePost::IssueThread/) {
		...
		(
			(
				...
				(
					...
					$self->session->form->process("class") eq "WebGUI::Asset::IssuePost::IssueThread"
				)
			) && 
...
sub getNewThreadUrl {
	my $self = shift;
	$self->getUrl("func=add;class=WebGUI::Asset::IssuePost::IssueThread");
}
...
sub getRssFeedItems {
  ...
  SELECT asset.assetId
    FROM IssueThread
         LEFT JOIN asset ON IssueThread.assetId = asset.assetId
         LEFT JOIN IssuePost ON IssuePost.assetId = IssueThread.assetId AND IssuePost.revisionDate = IssueThread.revisionDate
         LEFT JOIN assetData ON assetData.assetId = IssueThread.assetId
                                AND assetData.revisionDate = IssueThread.revisionDate
   WHERE asset.parentId = ? AND asset.state = 'published' AND asset.className = 'WebGUI::Asset::IssuePost::IssueThread'
...
sub getThreadsPaginator {
	...
	my $sql = "
		...
		from IssueThread 
			left join asset on IssueThread.assetId=asset.assetId 
			left join IssuePost on IssuePost.assetId=IssueThread.assetId and IssueThread.revisionDate = IssuePost.revisionDate 
			left join assetData on assetData.assetId=IssueThread.assetId and IssueThread.revisionDate = assetData.revisionDate 
		where 
...
sub recalculateRating {
    ...
    my ($count) 
        = $self->session->db->quickArray(
            "select count(*) from IssueThread 
                left join asset on IssueThread.assetId=asset.assetId 
                left join IssuePost on IssueThread.assetId=IssuePost.assetId 
                    AND IssueThread.revisionDate = (SELECT MAX(revisionDate) FROM IssueThread t WHERE t.assetId=asset.assetId)
    ...
    my ($sum) 
        = $self->session->db->quickArray(
            "SELECT SUM(IssueThread.threadRating) 
                FROM IssueThread 
                LEFT JOIN asset ON IssueThread.assetId=asset.assetId 
                LEFT JOIN IssuePost ON IssueThread.assetId=IssuePost.assetId
                    AND IssueThread.revisionDate = (SELECT MAX(revisionDate) FROM IssueThread t WHERE t.assetId=asset.assetId)
