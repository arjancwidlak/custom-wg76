package WebGUI::Asset::IssuePost;
...
use WebGUI::Asset::IssuePost::IssueThread;
use WebGUI::Cache;
use WebGUI::Group;
use WebGUI::HTML;
use WebGUI::HTMLForm;
...
sub addChild {
	...
	my $properties = shift;
	my @other = @_;
	if ($properties->{className} ne "WebGUI::Asset::IssuePost") {
...
sub addRevision {
	...
	if ($threadId eq "") { # new post
		if ($newSelf->getParent->isa("WebGUI::Asset::Wobject::IssueCollaboration")) {
...
sub canEdit {
    ...
    if (  
        ...
        && $form->get("class") eq "WebGUI::Asset::IssuePost"
    ) {
...
sub definition {
    ...
    push(@{$definition}, {
        ...
        icon=>'post.gif',
        tableName=>'IssuePost',
        className=>'WebGUI::Asset::IssuePost',
...
sub DESTROY {
	my $self = shift;
	$self->{_thread}->DESTROY if (exists $self->{_thread} && ref $self->{_thread} =~ /IssueThread/);
	$self->SUPER::DESTROY;
}
...
sub getReplyUrl {
	...
	return $self->getUrl("func=add;class=WebGUI::Asset::IssuePost;withQuote=".$withQuote);
}
...
sub getThread {
	...
	unless (defined $self->{_thread}) {
		my $threadId = $self->get("threadId");
                if ($threadId eq "") { # new post
                        if ($self->getParent->isa("WebGUI::Asset::Wobject::IssueCollaboration")) {
                                $threadId=$self->getId;
                        } else {
                                $threadId=$self->getParent->get("threadId");
                        }
                }
                $self->{_thread} = WebGUI::Asset::IssuePost::IssueThread->new($self->session, $threadId);
	}
	return $self->{_thread};	
}
...
sub purge {
        my $self = shift;
        my $sth = $self->session->db->read("select storageId from IssuePost where assetId=".$self->session->db->quote($self->getId));
...
sub setParent {
        my $self = shift;
        my $newParent = shift;
        return 0 unless ($newParent->get("className") eq "WebGUI::Asset::IssuePost" || $newParent->get("className") eq "WebGUI::Asset::IssuePost::IssueThread");
...
sub www_edit {
	...
	if ($func eq "add" || ($func eq "editSave" && $form->process("assetId") eq "new")) { # new post
        ...
        $action       = $self->getUrl if($func eq "editSave" && $className ne "WebGUI::Asset::IssuePost::IssueThread");
        ...
        elsif ($className eq "WebGUI::Asset::IssuePost::IssueThread") { # new thread
...
sub www_rate {	
	my $self = shift;
	$self->WebGUI::Asset::IssuePost::rate($self->session->form->process("rating")) if ($self->canView && !$self->hasRated);
	$self->www_view;
}
