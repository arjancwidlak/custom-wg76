package WebGUI::Asset::IssuePost::IssueThread;
...
our @ISA = qw(WebGUI::Asset::IssuePost);
...
sub definition {
	...
	my $i18n = WebGUI::International->new($session,"Asset_Thread");
		...
		icon=>'thread.gif',
                tableName=>'IssueThread',
                className=>'WebGUI::Asset::IssuePost::IssueThread',
                properties=>{
			subscriptionGroupId => {
				noFormPost=>1,
				fieldType=>"hidden",
...
sub getLastPost {
	...
	my $lastPost;
	if ($lastPostId) {
		$lastPost = WebGUI::Asset::IssuePost->new($self->session, $lastPostId);
	}
...
sub getNextThread {
    ...
    unless (defined $self->{_next}) {
        ...
        my ($id, $class, $version) = $self->session->dbSlave->quickArray(<<END_SQL, [$self->get("parentId"), $self->getId, $sortByCompare, $self->session->scratch->get("versionTag"), $self->session->user->userId]);
            ...
            from IssueThread
                left join asset on asset.assetId=IssueThread.assetId 
                left join assetData on assetData.assetId=IssueThread.assetId and assetData.revisionDate=IssueThread.revisionDate
                left join IssuePost on IssuePost.assetId=assetData.assetId and assetData.revisionDate=IssuePost.revisionDate
            where
                asset.parentId=?
                and asset.assetId <> ?
                and asset.state='published'
                and asset.className='WebGUI::Asset::IssuePost::IssueThread'
...
sub getPosts {
    ...
    $self->getLineage(["self","descendants"], {
        returnObjects       => 1,
        includeArchived     => 1,
        includeOnlyClasses  => ["WebGUI::Asset::IssuePost","WebGUI::Asset::IssuePost::IssueThread"],
    });
}
...
sub normalizeLastPost {
	my $self = shift;
	# Hmm.  Is this right?
	my ($lastPostId, $lastPostDate) = $self->session->db->quickArray("SELECT a.assetId, a.creationDate FROM asset as a INNER JOIN IssuePost as t ON a.assetId = t.assetId WHERE t.threadId = ? ORDER BY a.creationDate DESC LIMIT 1", [$self->getId]);
...
sub setParent {
        my $self = shift;
        my $newParent = shift;
        return 0 unless ($newParent->isa("WebGUI::Asset::Wobject::IssueCollaboration"));
...
sub trash {
    ...
    if ($self->getParent->get("lastPostId") eq $self->getId) {
        ...
        my ($id, $date) = $self->session->db->quickArray("select assetId, creationDate from asset where 
            lineage like ? and assetId<>? and asset.state='published' and className like 'WebGUI::Asset::IssuePost%' 
...
sub view {
    ...
    my $by = $self->session->form->process('by');
    my $paginatorVars = "by=$by" if $by;

    my $p = WebGUI::Paginator->new( $self->session, $self->getUrl( $paginatorVars ), $self->getParent->get("postsPerPage") );
	my $sql = "select asset.assetId, asset.className, assetData.revisionDate as revisionDate, assetData.url as url from asset 
		...
		left join IssuePost on IssuePost.assetId=assetData.assetId and IssuePost.revisionDate=assetData.revisionDate
		
		where asset.lineage like ".$self->session->db->quote($self->get("lineage").'%')
		."	and asset.state='published'
        and asset.className like 'WebGUI::Asset::IssuePost%'
...
#----------------------------
    my $orderBy = "assetData.revisionDate desc"; # if $self->session->form->process('by') eq 'date';
    $orderBy = "rand()" if $self->session->form->process('by') eq 'random';
    $orderBy = "IssuePost.rating desc" if $self->session->form->process('by') eq 'rating';
...
#	}
    ...
    $var->{'replies.areSortable'}   = $self->get('replies') > 1;
    $var->{'inclusiveRating'}       = $self->get('replies') + $self->get('threadRating');
#-----------------------------	
...
####	$p->setDataByQuery($sql, undef, undef, undef, "url", $currentPageUrl);
	...
	foreach my $dataSet (@{$p->getPageData()}) {
		next unless ($dataSet->{className} eq "WebGUI::Asset::IssuePost" || $dataSet->{className} eq "WebGUI::Asset::IssuePost::IssueThread"); #handle non posts!
		my $reply = WebGUI::Asset::IssuePost->new($self->session, $dataSet->{assetId}, $dataSet->{className}, $dataSet->{revisionDate});
