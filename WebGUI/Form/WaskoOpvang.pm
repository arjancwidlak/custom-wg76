package WebGUI::Form::WaskoOpvang;
...
sub definition {
	...
	push(@{$definition}, {
		label=>{
			defaultValue=>'Soort opvang'
			},
		name=>{
			defaultValue=>"WaskoOpvang"
			},
		defaultValue=>{
...
sub getOptions {
    ...
    my %locaties = $self->session->db->buildHash('
                SELECT distinct(`field_aoBNZg-hNvK-qULoO96c0w`),`field_aoBNZg-hNvK-qULoO96c0w`  
